...
flowchart TB
  subgraph UI["UI - Voice Picker page"]
    ...
    DEBUG["Debug status - phase, mic level, threshold"]
  end
...
- TTS completion reopens the microphone. The debug panel distinguishes local VAD transitions from
  Gemma-backed worker responses.
